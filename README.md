...
> - 자세 유지 안정  
### 주요 기능
> - 로봇의 기울기 및 자세 측정(I2C통신) 
> - 실시간 밸런싱 및 좌우 모터 속도 및 방향 제어를 통해 안정적인 상태 유지
> - 로봇의 기울기 각도나 수행 중인 명령에 따른 표정 출력(SPI통신)을 통해 사용자와의 원활한 상호작용 가능
...
### 모터 출력 제어
#### 1️⃣PWM 신호 기반의 정밀한 모터 제어
> STM32 MCU에서 출력하는 4개의 PWM 신호를 통해 모터의 방향, 속도 제어<br/>
...
#### 1️⃣이중 PID제어
> 두 제어를 합치면 속도를 높이려다 로봇이 넘어지거나, 중심을 잡으려다 속도가 멋대로 변할 수 있기 때문에 분리<br/>
> 외부 제어(엔코더)에서 현재 속도와 목표 속도를 기반으로 목표 기울기를 결정<br/>
> 내부 제어(IMU)에서 현재 기울기와 목표 기울기를 기반으로 모터를 돌릴 출력을 계산
...
> 블루투스 모듈과 UART 통신을 통해 스마트폰과 로봇 간 무선 통신 구현<br/>
...
### PID 제어 알고리즘
#### 1️⃣비선형 P 제어
> 오차가 작을 때는 부드럽게 제어, 오차가 클 때는 오차의 제곱에 비례하는 힘을 추가하여 강력한 힘을 주어 균형을 유지

#### 2️⃣안티 와인드업
> 적분항(I_term)이 과도하게 쌓여 로봇이 과도하게 움직이는 것을 막기 위해 한계치를 제한 <br/>
> 로봇이 밸런스를 잡은 후 오차 방향이 반대로 되면 즉시 적분항을 초기화함
</details>
